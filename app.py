...
async def process_start_command(message: types.Message):
    await message.answer("Выберите язык", reply_markup=kb.markup2)
...
async def order(message: types.Message):
    await message.answer("Выберите товар", reply_markup=kb.drink_kb_full)
...
async def shop(message: types.Message, state: FSMContext):
    ...
    await bot.send_message(message.from_user.id, 'Выберите количество', reply_markup=kb.count_kb_full)
...
async def count(message: types.Message, state: FSMContext):
    ...
    cursor.close()
    connect.commit()
    await message.answer('Добавлено в корзину!', reply_markup=kb.done_kb)
    connect.close()
...
async def buy(mess: types.Message):
    connect = sqlite3.connect('cards.db')
    cursor = connect.cursor()
    tg_id_bd = mess.from_user.id
    data = cursor.execute("""SELECT * FROM cards WHERE user_id=(?)""", (tg_id_bd, ))
    drink = None
    drink_count = 0
    tg_id = None
    for i in data:
    ...
    # # # connect.commit()
    # # # connect.close()
    price = LabeledPrice(label=drink, amount=int(drink_count))
    print(price)
